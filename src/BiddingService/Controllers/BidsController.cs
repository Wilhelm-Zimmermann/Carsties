...
public class BidsController : ControllerBase
{
    ...
    public async Task<ActionResult<BidDto>> PlaceBid(string auctionId, int amount)
    {
        ...
        if(auction == null)
        {
            ...
            auction = _grpAuctionClient.GetAuction(auctionId);
            if (auction == null)
                return BadRequest("Cannot accept bids to this auction");
        }

        if(auction.Seller != User.Identity.Name)
        {
            return BadRequest("The seller is different");
        }
